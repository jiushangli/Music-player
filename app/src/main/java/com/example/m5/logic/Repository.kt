...
object Repository {
    ...
    //查询二维码扫码情况
    fun getCodeStatue(key: String, timestamp: String) = liveData(Dispatchers.IO) {
        val result = try {

            val QrStatus = MusicNetwork.getCodeStatue(key, timestamp)
    ...
    //获取登录状态
    fun getStatus(timestamp: String) = liveData(Dispatchers.IO){
        val result = try {

            val status = MusicNetwork.getStatus(timestamp)
            if (status != null){
